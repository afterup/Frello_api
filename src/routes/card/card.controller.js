...
export async function fetchCard(req, res) {
  try{
      ...
      if(card) { return res.status(201).send({ card: card });}
      else { return res.status(403).send({ error: { message: 'FORBIDDEN' } })};
  }catch(err) {
      return res.status(500).send({ error: { message: err.message } });
  }
}
...
export async function updateCard(req, res) {
  ...
  try{
      ...
      const card = await checkExistId({ card_id: req.params.id, model : Card });
...
export async function deleteCard(req, res) {
  try{
      const card = checkExistId({ card_id: req.params.id, model : Card });
